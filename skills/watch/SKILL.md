...
**Auth-walled sources** (e.g. workspace-restricted Loom videos): handled automatically - the downloader retries with the browser's cookies (`--cookies-from-browser chrome`) when the first anonymous pass produces no video, so they download as long as the user is logged in to that site in Chrome. Set `WATCH_COOKIES_FROM_BROWSER=<browser>` (default `chrome`; also `firefox`/`safari`) to force cookies on the first pass or pick a different browser. See Failure modes.
...
- **Download fails / auth wall** → `download.py` auto-retries once with `--cookies-from-browser chrome` when the first anonymous pass produces no video, so workspace-restricted sources (e.g. a Loom shared within a workspace) download as long as the user is logged in to that site in Chrome. `WATCH_COOKIES_FROM_BROWSER=<browser>` (default `chrome`) forces cookies on the first pass or selects another browser (`firefox`/`safari`). If it STILL produces no video, the user is not logged in to the source in that browser (or it is region-locked): tell them plainly and do not keep retrying.
