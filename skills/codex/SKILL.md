---
name: codex
description: Use when the user asks to run Codex CLI (codex exec, codex resume), references OpenAI Codex for code analysis, refactoring, or automated editing, or wants a cross-model second-opinion review of a diff ("get Codex to review this", "cross-model review")
---
...
## Cross-Model Review (second opinion on a diff)

Use before merging a complex change or when extra confidence is wanted. Not
for trivial diffs.

1. **Gather the diff**: feature branch -> `git diff <integration>...HEAD`;
   uncommitted -> `git diff`. Summarize what it does and why.
2. **Send to Codex** (read-only sandbox): "Review this diff for bugs, security
   issues, logic errors, and missed edge cases. Be critical - flag anything
   suspicious." + the diff.
3. **Reconcile each finding** - agree (flag to user as real), disagree
   (explain with evidence from the actual code), or uncertain (present both).
   Apply the Critical Evaluation guidelines below; never defer blindly.
4. **Report unified**: issues both models agree on (high confidence), issues
   only Codex flagged (user judgment), issues you caught that Codex missed.
